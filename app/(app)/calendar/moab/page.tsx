  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        const moab = props.find((p) => p.name.toLowerCase().includes('moab'))
        if (!moab) {
          setError('Moab property not found. Set it up in Settings.')
          setLoading(false)
          return
        }
        setProperty(moab)

        // Fetch rules engine output and calendar data in parallel
        const [rulesRes, calRes] = await Promise.all([
          fetch('/api/rules-engine', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ property_id: moab.id }),
          }),
          fetch(`/api/calendar-data?property_id=${moab.id}`),
        ])
